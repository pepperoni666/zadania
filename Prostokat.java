...
public class Prostokat implements Figura{
	...
	public String toString(){
		return "Prostokat; obwod: " + obwod() + "; pole: " + pole() + "; punkt: (" + punkt.getPozX() + ", " + punkt.getPozY() + ");";
	}
	...
	public boolean przecina(Kolo x){
		...
		if(Math.pow((Math.pow((x.punkt.getPozX() - ix), 2) + Math.pow((x.punkt.getPozY() - iy), 2)), 1/2) < (x.bok+(Math.pow((Math.pow(bokA, 2) + Math.pow(bokA, 2)), 1/2)/2)))
			return true;
		else
			return false;
	}
